...
const GRID = 16; // logical cells per tile edge - matches the ground tile grid, so tree tiles align 1:1 with it
const BLOCK = 2; // real pixels per grid cell -> 32x32 per tile
const CELL_PX = GRID * BLOCK;

// LOG_CENTER is the grid-cell center buildLogGrid masks a trunk from, at `(GRID - 1) / 2`.
// BLOB_CENTER is a second, `GRID / 2` convention used only by buildBounds's own blob-coordinate
// normalization below (the round trunk's collision hull) - it isn't a true radial-blob grid
// origin here the way it is in gen-tundra-structures.mjs/gen-desert-structures.mjs, which this
// otherwise mirrors.
 ...
 * result. Kept tight to the trunk's actual silhouette rather than the whole
 * tile - see `SpriteBounds`.
 *
 ...
 * "square" log variant). A finite `radius` masks it into a circle instead
 * (the "round" log variant); cells outside that circle use `cornerLeaves`'
 * palette/gap-chance rather than staying transparent, so a diagonally
 * adjacent leaf tile visually connects into the trunk tile's corners
 * instead of leaving a bare gap of ground showing.
 *
...
// A "round" trunk's own collision hull: a tight circle at LOG_RADIUS. The "square" variant
// fills its tile edge-to-edge instead (see TREE_ROWS below), so it's left with no authored
// bounds of its own and just falls back to the whole tile square, which is already exactly
// its silhouette.
...
const rowDescriptors = TILE_ROWS.flatMap((tiles, row) => tiles.map((tile, column) => {
    ...
    return {
        ...
        x,
        y,
        interactable: Boolean(tile.bounds), // decorative props (leaves, square logs) aren't; round trunks are
...
const { outPath, descriptorOutPath } = parseCliArgs("gen-forest-structures.mjs", "static/forest-structures.json");
writeSpriteSheet(outPath, descriptorOutPath, sheetW, sheetH, sheet, descriptor);
console.log("forest structure tiles:", TILE_ROWS.map((row) => row.map((tile) => tile.type).join(", ")).join(" | "));
