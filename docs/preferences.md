...
- Every interactable sprite (cactus, boulder, tree trunk, skeletal tree,
  ...
  See `scripts/gen-forest-structures.mjs`/`gen-tundra-structures.mjs`/
  `gen-desert-structures.mjs`'s `buildBounds`/`buildBoulderPieceBounds` for
  the pattern (sample the drawn shape's own geometry - blob circles, a
  composite boulder's shared circle, etc. - and hull/clip it), and
  `World.structureCollisionPolygonForSprite`
