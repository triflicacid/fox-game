 ...
 * while checked > `inputStyle` (falls back to `style`) at rest.
 */
export interface CheckboxInput extends InputBase {
    ...
    content: TextSegment[];
    /** Style colouring just the box while checked. Falls back to `selectedStyle`. */
    inputSelectedStyle?: TextStyle;
    /** Style colouring just the box at rest (unchecked, unfocused). Falls back to `style`. */
