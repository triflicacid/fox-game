...
const display = new InteractableDisplay({}, WIN98_THEME, "always");
...
function buildLines(): DisplayLine[] {
    return [
        ...
        line().content("Use Tab / arrows / Enter, or click directly."),
        line().content(hr()),
        ...
        line().content(`Hello ${state.username || "friend"}. speed=${state.speed}, palette=${state.palette}, debug=${state.debugHud ? "on" : "off"}, pings=${state.pingCount}`),
    ];
}
...
function render(): void {
    ...
    display.setClickRegion({
        x: PANEL_PADDING,
        y: PANEL_PADDING,
        w: canvas.width - PANEL_PADDING * 2,
        h: canvas.height - PANEL_PADDING * 2,
    });
    ...
    display.setFocusables(display.layoutLineFocusables(rows, contentX, contentY, LINE_SPACING));
